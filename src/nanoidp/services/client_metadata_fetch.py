...
class _PinnedConnection:
    ...
    def __enter__(self) -> "_PinnedConnection":
        ...
        try:
            ...
            raw.settimeout(_deadline_remaining(self.deadline))
            self._sock = context.wrap_socket(raw, server_hostname=self.hostname)
        except FetchRefused:
            raw.close()
            raise
...
def _acceptable_address(hostname: str, port: int, settings: Settings) -> str:
...
def cache_lifetime(cache_control: Optional[str]) -> Optional[float]:
    ...
    and ``no-cache`` mean this answer is not kept, and ``max-age`` gives a
    number that ``client_metadata.bounded_lifetime`` will clamp. Anything
    else is ignored, because revalidation is not implemented and pretending
    otherwise would be the part that goes wrong.
    """
    ...
    if "no-store" in directives or "no-cache" in directives:
        raise FetchRefused("the metadata document says it must not be stored")
...
def fetch_document(client_id: str, settings: Settings) -> Tuple[Any, Optional[float]]:
    ...
    address = _acceptable_address(hostname, port, settings)
