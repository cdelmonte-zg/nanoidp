...
**SAML detail**: a persona login can't claim `AuthnContextClassRef: PasswordProtectedTransport`, since no password was used - NanoIDP emits `urn:oasis:names:tc:SAML:2.0:ac:classes:unspecified` instead for sessions authenticated this way. A session that also completed the [declarative TOTP second factor](configuration.md#second-factor-totp) claims a third value, `urn:oasis:names:tc:SAML:2.0:ac:classes:TimeSyncToken`, instead of `PasswordProtectedTransport`.
