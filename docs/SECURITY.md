...
This matters because the management API (`/api/*`) is **unauthenticated by design** (see [MCP Server Security](#mcp-server-security) for the equivalent concern on the MCP side). Those endpoints can mint a validly signed access token for any user, including `admin`, rotate the signing keys, clear the audit log, and create runtime users and clients that authenticate on every protocol surface (`/api/runtime`, see [Disposable test identities](https://cdelmonte-zg.github.io/nanoidp/guides/runtime-identities.html)). Loopback binding keeps that surface off the network. `oauth.dynamic_registration` (off by default) adds an endpoint of a different kind: `POST /register` is open by design when enabled, so that a client can register itself, and `management_secret` does not gate it. Enabling it on a reachable instance lets anyone create OAuth clients on it, bounded only by `max_clients`; the clients are runtime state and never reach `settings.yaml` unless an operator promotes one. `max_clients` does not expire on its own, so anyone who can reach an enabled `/register` can fill it until an operator frees the slots or restarts; enabling `rate_limit_enabled` applies the token endpoint's rate to `/register` too. `oauth.client_id_metadata_documents` (off by default) is the other outward-facing opt-in: with it on, `/authorize` fetches a URL a client chose. Only hosts named in `allowed_hosts` are fetched at all, and only on the port named with them (443 when none is), only globally routable addresses are connected to, redirects are never followed, the body is capped at 5 KiB and the whole fetch at 5 seconds, and this process will make at most 30 such fetches a minute. Treat an `allowed_hosts` entry as the decision it is: a host and port named there is one this server will make requests to, at any path a client chooses. The refusal a client sees is uniform, but its timing is not, so an instance strangers can reach can still be used to tell an allowed host from a refused one. If you need it reachable by more than one host, also consider [`management_secret`](#management-secret), which requires a shared secret for these mutating calls specifically (reads stay open either way).
...
4. **Validation**: Tokens signed with previous keys remain valid until those keys are rotated out. That holds for a resource server that reads the JWKS and for NanoIDP's own endpoints alike (`/userinfo`, `/introspect`, `/revoke`, the refresh grant): a token is verified against the key its `kid` names, among the active key and the previous ones kept. A token that names a key that is no longer kept, or not one of this issuer's, is invalid; a token with no `kid` is verified against the active key. External keys are one key with no history: a token signed with it verifies whatever `kid` it names, so changing `jwt.external_key_id` for the same pair invalidates nothing.
5. **Several processes**: when NanoIDP processes share a keys directory, a rotation made by one is noticed by the others the next time they sign, verify or publish their JWKS: each looks at `kid.txt` (without taking the lock, with a `stat` first) and, when it names another key, loads that bundle whole; it never generates keys to answer a marker it cannot load, and says so in its log instead. Tokens they signed just before stay valid, their key being a previous one now.
