...
| `POST /api/keys/rotate` | Rotate cryptographic keys; `409` when `jwt.external_keys` is configured, since the operator's key is not nanoidp's to replace, and when this process cannot write the keys directory (`kind`: `keys_directory_not_writable`, or `lock_namespace_unavailable` when its lock file cannot even exist); `503` with `Retry-After` when the directory's lock, shared with other NanoIDP processes, cannot be had in time (`kind`: `lock_timeout`), or cannot be taken at all on this filesystem (`lock_unsupported`, which a retry does not fix). The refusals carry a fixed message; the directory they are about is in the server log |
