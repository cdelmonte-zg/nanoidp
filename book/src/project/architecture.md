...
| `services/sqlite_runtime_store.py` | The runtime store in a pair of SQLite files, for several NanoIDP processes on one host (#354): the repositories in one, the audit in another (`SqliteAuditStore`, one file has one writer and the audit must not wait for the repositories). One table for every repository, its rows in the order of creation; a decision is a `BEGIN IMMEDIATE` transaction; WAL with `synchronous=NORMAL`; one connection per thread, and none across a fork (an at-fork hook waits for the operations in progress and closes them all); the file and its `-wal` and `-shm` private, and a file that is not a NanoIDP runtime store refused. Chosen by `runtime.store: sqlite` and `runtime.path` (relative to the configuration directory) in `settings.yaml`. Owner leases (`runtime-owners/`) prove a promotion's writer dead, so that its claim can be recovered (#354) |
